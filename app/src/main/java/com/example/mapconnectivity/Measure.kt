...
//    @Query("SELECT (CASE COUNT(*) WHEN 0 THEN NULL ELSE AVG(lte) END) AS avgLte, (CASE COUNT(*) WHEN 0 THEN NULL ELSE AVG(wifi) END) AS avgWifi, (CASE COUNT(*) WHEN 0 THEN NULL ELSE AVG(db) END) AS avgDb FROM measures WHERE :lat1 >= lat AND :lat2 <= lat AND :lon1 >= lon AND :lon2 <= lon AND (imported = 0 OR imported = :imported) ORDER BY timestamp DESC")
